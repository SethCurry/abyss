import { createAgentSession, type ExtensionAPI } from "@earendil-works/pi-coding-agent";
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("golangci_lint", {
    ...
    handler: async (num_issues, ctx) => {
      ...
      const { stdout } = await exec("golangci-lint run --output.json.path=stdout --show-stats=false --issues-exit-code=0");
      const lintResult = JSON.parse(stdout) as GolangciLintResult;

      ctx.ui.notify(`done running linter, found ${lintResult.Issues.length} issues`, "info")
      ...
      if (issues.length > numIssues) {
        ...
        const issueSummaries = issues.map(issue => `${issue.Pos.Filename}:${issue.Pos.Line}:${issue.Pos.Column} ${issue.Text}`);
        const issueSummary = issueSummaries.join("\n");


      const shouldProceed = await ctx.ui.confirm("Have agent fix issues?", `I am going to try to fix ${issues.length} issues, should I proceed?\n\n${issueSummary}`);
